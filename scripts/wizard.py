...
def main():
    ...
    try:
        import subprocess
        gen_pdf = SKILL_DIR / "scripts" / "generate_pdf.py"
        subprocess.run([sys.executable, str(gen_pdf), str(out_path), str(pdf_path)],
                       capture_output=True, timeout=30)
        if pdf_path.exists():
            check(f"PDF: {pdf_path}")
        else:
            check("PDF generation failed — see errors above")
    except Exception as e:
        check(f"PDF generation failed: {e}")

    # Step 5: Auto-generate DOCX
    star("GENERATING DOCX")
    docx_path = out_path.with_suffix('.docx')
    try:
        gen_docx = SKILL_DIR / "scripts" / "generate_docx.py"
        subprocess.run([sys.executable, str(gen_docx), str(out_path), str(docx_path)],
                       capture_output=True, timeout=30)
        if docx_path.exists():
            check(f"DOCX: {docx_path}")
        else:
            check("DOCX generation failed — see errors above")
    except Exception as e:
        check(f"DOCX generation failed: {e}")
